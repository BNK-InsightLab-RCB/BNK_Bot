...
**개발 1단계 완료 (2026-09-06).** 적재·검색·정확성 통제까지 끝났고, 남은 것은 납품처 규격에
맞춰 껍데기(API 스키마·인증·로그 형식)를 갈아끼우는 일과 아래 [알려진 제약](#알려진-제약)입니다.

### 코퍼스
3,812문서 · 약 115,000쪽 · 9개 카테고리 중 **3,121문서(82%) / 234,050청크 적재 완료**
(펀드·예금·신탁·외환·전자금융·기타·대출·카드약관 전량 또는 95% 이상,
보험 설명서 99%, **보험 약관 4%**, **카드 설명서 45%** — 사유는 알려진 제약 참조)

### 측정 결과

| 항목 | 값 | 재생성 |
| --- | --- | --- |
| **검색 정확도** | **pass@1 86% · pass@5 91%** | `eval_retrieval_v2.py` (914문항) |
| **답변 정확도** | **29/33 (88%)** | `eval_answer.py` |
| **숫자 위반** | **0건** — 답변의 모든 수치가 근거 문서에 실재 | `eval_answer.py` |
| **거부 정확도** | **5/5** — 도메인 밖 질문에 환각 없음 | `eval_answer.py` |
| 무관 질문 게이트 | 정상 질문 38/38 통과 · 무관 9/10 차단 | `measure_score_threshold.py` |
| 청킹 글자 보존 | **3,760/3,762** | `check_chunk_conservation.py` |
| 응답 지연 | 도메인 밖 거부 **0초**(LLM 미호출) · 일반 답변 중앙 **14.6초** | `eval_answer.py` |

**답변 실패 4건은 전부 "답할 수 있는데 모른다고 함" 또는 "부분 답변"이며,
틀린 정보를 준 경우는 0건입니다.** 실패가 안전한 방향으로만 일어납니다.

`temperature=0` 이라 재실행해도 같은 값이 나옵니다.

### 평가셋에 대해 (인용 시 함께 밝혀야 할 조건)
- **검색 914문항** — 색인에 실제로 존재하는 상품명을 뽑아 **기계적으로 생성**했습니다.
  9개 카테고리 층화. 정확형 468(상품명 그대로) + 실전형 446(고객 말투 변형: 문서 꼬리표
  제거·띄어쓰기·줄임말). **문서 내용을 읽고 정답을 창작하지 않았으므로** 정답이 틀릴 수
  없습니다. 다만 *상품 탐색*만 재며, 질문에 상품명이 어떤 형태로든 들어 있습니다.
- **답변 33문항** — 자가채점(질문과 기대답을 직접 라벨). 회귀 확인용 게이트이지
  gold benchmark 가 아닙니다.

### 검색 정확도를 어떻게 올렸나 (56% → 86%)
| 조치 | pass@1 | 비용 |
| --- | --- | --- |
| 시작 (임베딩 검색만) | 56% | — |
| 어휘 필터 도입 (키워드로 범위 축소 + 임베딩으로 순위) | 65% | 인덱스 20초 |
| 파일명을 검색 대상에 추가 | 80% | 인덱스 2.6초 |
| 파일명 색인을 접두사 방식으로 | 82% | 인덱스 2.0초 |
| 평가 판정 정규화(측정 결함 제거) | **86%** | — |

효과를 낸 것은 **전부 인덱스 설정 변경**이며, 재적재·재임베딩은 필요하지 않았습니다.
반대로 cross-encoder 리랭커(2.1GB)는 **+2문항에 24배 지연**이라 도입하지 않았습니다.
...
flowchart LR
    ...
    RET --> LEX["어휘 필터<br/>상품명 토큰으로 후보 축소"]
    LEX --> QD["Qdrant<br/>단일 컬렉션 + payload 필터<br/>+ 전문검색 인덱스"]
    QD --> TH{"score ≥ 0.60?"}
...
│   ├── lexical.py       어휘 필터 — 질의에서 상품명 토큰을 골라 후보를 좁힌다
│   ├── reranker.py      cross-encoder 재순위 (측정 결과 기본 비활성)
...
| 지점 | 통제 | 효과 |
| --- | --- | --- |
| 입력 | 검색 score 하한(0.60) | 무관 질문이 LLM 에 닿기 전에 거부. 도메인 밖 질문 **0.0초** 응답 |
...
```bash
python scripts/build_eval_set.py --per-category 60   # 평가셋 생성(시드 고정 · 914문항)
python scripts/eval_retrieval_v2.py         # 검색 품질 — 카테고리 층화 · 전략 A/B 비교
...
python scripts/eval_retrieval.py            # 검색 품질(옛 15문항 · 내용 이해까지 판정)
python scripts/ab_context_header.py         # 컨텍스트 헤더 A/B/C 실험
```

적재·청킹·프롬프트를 건드렸으면 최소 앞의 네 개는 돌려 회귀를 확인합니다.

> ⚠️ **평가 스크립트에 검색 로직을 다시 구현하지 마세요.** `Retriever` 를 그대로 호출해야
> 운영 경로를 측정합니다. 이 함정 때문에 검색을 고치고도 수치가 그대로여서
> "개선 효과 없음"으로 오판할 뻔한 적이 두 번 있었습니다.

### 대량 적재 도구

```bash
python scripts/prewarm_parse.py <경로> --workers 5   # 파싱만 미리 병렬로(19.8h → 1.5h)
bash scripts/ingest_batched.sh "보험/약관" 100        # 100건씩 끊어 적재(메모리 반환)
bash scripts/resume_ingest.sh                        # 재부팅 후 재개
```

한 프로세스로 대량을 연속 처리하면 힙이 누적되어 느려집니다(실측 2.2분/건 → 7.0분/건).
배치마다 프로세스를 새로 띄우면 종료 시 메모리가 통째로 반환됩니다.
...
| 항목 | 계획 | 현재 | 비고 |
| --- | --- | --- | --- |
| Hybrid Search (BM25 + Dense) | Elasticsearch 기반 | **구현됨 (방식이 다름)** | 키워드로 후보를 좁히고 임베딩이 순위를 정하는 방식. pass@1 56%→86%. 아래 설명 참조 |
| RRF 병합 | BM25/Vector 순위 병합 | **도입하지 않음** | 실측에서 **악화**(53%→40%). Qdrant 전문검색은 불리언 필터라 랭킹이 없어, 임의 순서 후보를 융합하면 잡음이 상위로 올라옴 |
| Reranker | Top 3~5 재정렬 | **도입하지 않음** | cross-encoder(bge-reranker-v2-m3)로 실측: **+2문항에 24배 지연**(0.17초→4.13초). 코드는 남겨 두었고 설정 한 줄로 켤 수 있음 |
| 진짜 BM25 (sparse vector) | | 미도입 | 컬렉션 재생성이 필요하나 **재임베딩은 불필요**(기존 벡터 재사용, 이관 수십 분) |
| 검색 지표 | Recall@3/@5, MRR | pass@1/@5 | 지표 체계가 다름. 정식 평가셋 구축 시 정렬 필요 |

> **Hybrid 를 "RRF 로 융합"이라고 부르는 통상적 방식과 다릅니다.** 이 프로젝트에서는
> 질의에서 상품명일 가능성이 높은 토큰을 골라 **파일명·본문 전문검색으로 후보를 좁힌 뒤**,
> 순위는 임베딩이 정합니다. 교과서적 RRF 융합은 실측에서 오히려 나빠져 기각했습니다.
> 근거와 기각된 변형 목록은 `src/chat/lexical.py` 주석에 있습니다.
...
| 최신/폐기 문서 구분 | 폐기 문서를 검색에서 제외 | **판단하지 않고 전부 색인** | **측정으로 불가 판정.** 파일명으로 최신 판단은 32%만 가능하고 그마저 오분류 위험(TDF2030/2040/2060 은 판본이 아니라 별개 상품). 문서 본문의 시행일 추출은 전체 22%·**보험 2%**(안내장이라 시행일을 안 적음). 대신 `source_file` 을 응답에 노출해 판단을 상위로 넘김 |
...
- **적재가 82%(3,121/3,812문서)에서 멈춰 있습니다.** 엔진 결함이 아니라 개발 머신(18GB RAM)의
  메모리 한계입니다. 남은 보험 약관 615건은 전체 페이지의 60%를 차지해 완주에 30시간 이상이
  필요합니다. **보험 약관 세부 조항을 물으면 "모른다"고 답하는 것이 현재의 정상 동작입니다.**
- **카드 안내장 59건은 본문이 11~500자에 불과합니다.** 디자인 리플렛이라 글자가 벡터 도형으로
  그려져 텍스트 추출이 되지 않습니다. 해법(macOS Vision OCR)은 검증을 마쳤으나 해당 문서
  재적재가 필요해 별도 작업으로 분리했습니다. 검색은 이 문서들을 정확히 찾아내며,
  내용이 없어 답하지 못할 뿐입니다.
- **같은 상품의 판본이 여러 개 색인되어 있습니다**(보험 37% · 예금 24% · 펀드 8%).
  고객이 판본을 특정하지 않으면 그중 하나가 선택되며, **어느 것이 최신인지 시스템은
  판단하지 않습니다**(위 "최신/폐기 문서 구분" 참조). 응답의 `source_file` 로 확인해야 합니다.
  → **문서 메타데이터(시행일·유효 여부)를 함께 제공받으면 최신판 우선 제공이 가능합니다.**
- **응답 지연 중앙값 14.6초**입니다. 대부분이 로컬 9B 모델의 생성 시간이며, GPU 서버에서는
  크게 줄어듭니다.
- **답변 평가셋이 소규모 자가채점(33문항)** 입니다. 회귀 게이트이지 gold benchmark 가 아닙니다.
  검색 평가셋(914문항)은 기계 생성이라 정답은 정확하지만 *상품 탐색*만 측정합니다.
...
- **검색 임계 0.60 은 48개 질문 표본(정상 38 · 무관 10)으로 보정한 값**입니다. 코퍼스가 크게
  바뀌면 `scripts/measure_score_threshold.py` 로 재보정해야 합니다. 23만 청크 규모에서는
  정상/무관 질문의 점수 분포가 겹쳐 완전 분리가 불가능하므로, **정상 질문 무손실을 제약으로
  무관 질문 차단을 최대화**하는 기준을 씁니다(현재 정상 38/38 통과 · 무관 9/10 차단).
  통과한 무관 질문은 LLM 프롬프트와 숫자 가드레일이 2차로 거릅니다.
