...
이 프로젝트는 SQL 생성 챗봇이 아니라, 내부 문서를 검색하고 근거 문서에 기반해 상담 답변을 생성하는 AI 상담 시스템을 목표로 합니다. 외부 API에 의존하지 않는 로컬 LLM, Elasticsearch 기반 Hybrid Search, 권한 제어와 민감정보 마스킹을 포함한 보안 중심 구조를 설계합니다.
...
| 출처 문서 표시 | 답변에 사용된 문서명, 섹션, 버전 등 근거 정보 표시 | 설계 |
...
| 영역 | 기술 후보 | 비고 |
| --- | --- | --- |
| Frontend | React 또는 Vue | 현재 구현 전, 상담 UI 후보 |
| Backend | Spring Boot | 인증, 권한, 상담 이력, 로그, 관리자 기능, FastAPI 호출 |
| AI Server | FastAPI | RAG 검색, 임베딩, Reranker, LLM 호출, STT 처리 |
| Search Engine | Elasticsearch | BM25, dense_vector, 권한 필터링 |
| Embedding | `nlpai-lab/KURE-v1`, `BAAI/bge-m3` | 한국어 업무 문서 중심 검증 |
| LLM | Qwen / Llama 계열 로컬 모델 | 내부망 또는 로컬 환경 실행 |
| STT | whisper.cpp | 향후 음성 상담 UX 확장 |
...
2. OCR 오류, 반복 헤더/푸터, 표 구조, 폐기 문서 등을 정제합니다.
...
| OCR/PDF 추출 오류 정제 | 깨진 문자, 잘못된 줄바꿈, 페이지 번호 혼입 등을 수정 |
...
| 모델 | 검토 기준 |
| --- | --- |
| `nlpai-lab/KURE-v1` | 한국어 은행 매뉴얼, 규정, 절차 문서 중심일 때 우선 검토합니다. 한국어 질의와 한국어 업무 문서 간 검색 품질을 중점 평가합니다. |
| `BAAI/bge-m3` | 한국어/영어 혼합 문서, SWIFT, AML, KYC, 해외송금 등 다국어 문서가 많은 경우 비교 대상으로 검토합니다. |

최종 임베딩 모델은 감각적으로 선택하지 않고 내부 테스트셋을 기반으로 검증합니다. 주요 지표는 `Recall@3`, `Recall@5`, `MRR`, Top-k 근거 정확도, 오검색률입니다.
...
음성 입력은 향후 확장 기능입니다. 메뉴형 챗봇을 음성 기반 대화형 상담 UX로 확장하기 위해 whisper.cpp를 검토합니다.
...
STT는 상담 UI의 입력 채널을 확장하는 역할이며, 답변 생성 로직은 기존 RAG 파이프라인과 Guardrail 정책을 그대로 따릅니다.
...
| 경로 | 역할 |
| --- | --- |
| `frontend/` | 상담 화면, 출처 표시, 음성 입력 UI |
| `backend-spring/` | 인증, 권한, 상담 이력, 로그, 관리자 기능, FastAPI 연동 |
| `ai-server/` | FastAPI 기반 RAG/LLM/STT 처리 서버 |
...
| 출처 표시 정확도 | 답변에 표시된 출처가 실제 사용 근거와 일치하는지 평가 |
